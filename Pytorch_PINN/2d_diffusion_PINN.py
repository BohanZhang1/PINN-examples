...
def geo_train(device, x_in, y_in, xb, yb, cb, xb_Neumann, yb_Neumann, batchsize, learning_rate, epochs, path,
              Flag_batch, f_source, Diff, Flag_BC_exact, Lambda_BC):
    if (Flag_batch):
    ...
    def Loss_BC(xb, yb, xb_Neumann, yb_Neumann, cb):
        ...
        c_y = torch.autograd.grad(c_n, yb_Neumann, grad_outputs=torch.ones_like(yb_Neumann), create_graph=True,
                                  only_inputs=True)[0]
...
geo_train(device, x, y, xb, yb, cb, xb_Neumann, yb_Neumann, batchsize, learning_rate, epochs, path, Flag_batch,
          f_source, Diff, Flag_BC_exact, Lambda_BC)
# tic = time.time()
