...
def geo_train(device, x_in, y_in, xb, yb, ub, vb, batchsize, learning_rate, epochs, path, Flag_batch, Diff, rho,
              Flag_BC_exact, Lambda_BC, nPt, T, xb_inlet, yb_inlet, ub_inlet, vb_inlet):
    if (Flag_batch):
        ...
        def forward(self, x):
            ...
            if (Flag_BC_exact):
                output = output * (x - xStart) * (y - yStart) * (y - yEnd) + U_BC_in + (y - yStart) * (
                        y - yEnd)  # modify output to satisfy BC automatically #PINN-transfer-learning-BC-20
    ...
    def criterion_psi(x, y):
        ...
        psi_xxx = torch.autograd.grad(psi_xx, x, grad_outputs=torch.ones_like(x), create_graph=True, only_inputs=True)[
            0]
        psi_yyy = torch.autograd.grad(psi_yy, y, grad_outputs=torch.ones_like(y), create_graph=True, only_inputs=True)[
            0]
        psi_xyy = torch.autograd.grad(psi_xy, y, grad_outputs=torch.ones_like(y), create_graph=True, only_inputs=True)[
            0]
        psi_yxx = torch.autograd.grad(psi_xy, x, grad_outputs=torch.ones_like(x), create_graph=True, only_inputs=True)[
            0]
        ...
        loss_2 = psi_y * psi_xy / X_scale - psi_x * psi_yy - Diff * (psi_yxx / X_scale2 + psi_yyy) + 1 / rho * (
                P_x / X_scale)  # X-dir
        loss_1 = - psi_y * psi_xx / X_scale2 + psi_x * psi_xy - Diff * (
                -psi_xxx / X_scale3 - psi_xyy / X_scale) + 1 / rho * P_y  # Y-dir
...
geo_train(device, x, y, xb, yb, ub, vb, batchsize, learning_rate, epochs, path, Flag_batch, Diff, rho, Flag_BC_exact,
          Lambda_BC, nPt, T, xb_inlet, yb_inlet, ub_inlet, vb_inlet)
# tic = time.time()
